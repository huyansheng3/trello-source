...
export interface CompactBoardTileUnconnectedProps {
...
export const CompactBoardTileUnconnected: React.FunctionComponent<CompactBoardTileUnconnectedProps> = ({
  board,
  boardTileClassName,
  focused,
  isDragging,
...
interface CompactBoardTileProps {
...
export const CompactBoardTile = ({
  idBoard,
  ...rest
}: CompactBoardTileProps) => {
  const board = useSelector((state: State) => getBoardById(state, idBoard));
  const teamName = useSelector((state: State) => {
    const team =
      board && board.idTeam ? getTeamById(state, board.idTeam) : undefined;

    return team ? team.displayName : undefined;
  });

  const isStarred = useSelector((state: State) =>
    isBoardStarred(state, idBoard),
  );

  const isBoardTemplate = useSelector((state: State) =>
    isTemplate(state, idBoard),
  );

  return board ? (
    <CompactBoardTileUnconnected
      board={board}
      teamName={teamName}
      isStarred={isStarred}
      isTemplate={isBoardTemplate}
      {...rest}
    />
  ) : null;
};
