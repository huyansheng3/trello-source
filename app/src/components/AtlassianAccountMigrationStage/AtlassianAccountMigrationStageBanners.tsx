...
export const AtlassianAccountMigrationStageBanners: React.FC = ({
  ...
  const AtlassianAccountMigrationConfirmation = useLazyComponent(
    () =>
      import(
        /* webpackChunkName: "atlassian-account-migration-confirmation" */ 'app/src/components/AtlassianAccountMigration'
      ),
    { namedImport: 'AtlassianAccountMigrationConfirmation', preload: false },
  );
  ...
  if (!shouldRenderBanner && !shouldRenderConfirmation && !shouldRenderPrompt) {
    return null;
  }
  ...
  const renderConfirmation = () => {
    if (!shouldRenderConfirmation) {
      return null;
    }

    switch (stage) {
      case Stage.enterpriseMigration:
      case Stage.inactiveMigration:
      case Stage.migrationSuccess:
        return (
          <AtlassianAccountMigrationConfirmation
            forceShow={!!forceStage}
            wasActiveMigration={stage === Stage.migrationSuccess}
            wasEnterpriseMigration={stage === Stage.enterpriseMigration}
          />
        );
      default:
        return null;
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>{renderConfirmation()}</Suspense>
