...
import { CardContents } from 'app/src/components/CanonicalComponents';
...
export const CreateCardFromTemplatePopover: React.FunctionComponent<CreateCardFromTemplateProps> = ({
  ...
  const onChangeList = useCallback(
    ({ value }: typeof listOptions[0]) => setSelectedListId(value),
    [setSelectedListId],
  );
  const onClickEdit = useCallback(() => {
    Analytics.sendClickedButtonEvent({
      buttonName: 'editCardTemplatesButton',
      source: 'cardTemplatesPopover',
      containers: {
        card: {
          id: idCard,
        },
  ...
  const hasCustomFields = cardTemplate.customFieldItems.length > 0;
      ...
      {showKeepSection && (
        <>
          ...
          {hasCustomFields && (
            <KeepFromSourceCheckbox
              disabled={isCreatingCard}
              id="idKeepCustomFields"
              checked={keepCustomFields}
              setChecked={setKeepCustomFields}
              labelKey="custom-fields"
              count={cardTemplate.customFieldItems.length}
            />
          )}
          {hasMembers && (
            <KeepFromSourceCheckbox
