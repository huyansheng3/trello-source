...
import { getDateWithAddedMonths } from '@trello/dates';
import { EventData, EventSegment } from './types';

// The Dates.getEndOfWeek method subtracts a day if the `endOf`
// method returns a Sunday date. This means that the function will
// always try to return the Saturday date. For some locales (e.g.
// Fr), the end of the week should be Sunday.
export const getEndOfWeek = (date: Date) => {
  return moment(date).endOf('week').toDate();
};

/**
 * Returns the first VISIBLE day for a month view.
 *
 * This will look at the week that includes the first day of
 * the month and returns the first day of that week (this
 * will not necessarily be the 1st of the month, e.g. this would
 * return August 29th for a date in September 2021).
 */
export const getFirstVisibleDate = (date: Date) => {
  const firstOfMonth = Dates.getFirstOfMonth(date);
  return Dates.getStartOfWeek(firstOfMonth);
};

/**
 * Returns the last VISIBLE day for a month view.
 *
 * This will look at the week that includes the last day of
 * the month and returns the last day of that week (this
 * would return October 2ng for a date in September 2021).
 */
export const getLastVisibleDate = (date: Date) => {
  const lastOfMonth = Dates.getLastOfMonth(date);
  return getEndOfWeek(lastOfMonth);
};
...
export const getDaysToRenderForMonth = (date: Date) => {
  const firstVisibleDay = getFirstVisibleDate(date),
    lastVisibleDay = getLastVisibleDate(date);
...
export const getInitialWorkspaceCalendarDateRange = (): [Date, Date] => {
  const startOfRange = getFirstVisibleDate(
    getDateWithAddedMonths(new Date(), -1),
  );
  const endOfRange = getLastVisibleDate(getDateWithAddedMonths(new Date(), 1));
  return [startOfRange, endOfRange];
};

export const formatCalendarDateRangeToString = (
  dateRange: [Date, Date],
): string => {
  return `${moment(dateRange[0]).format()}...${moment(dateRange[1]).format()}`;
};
