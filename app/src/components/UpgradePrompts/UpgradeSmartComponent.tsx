...
export type PromptId =
  | 'attachmentSizeLimitUpgradePromptFull'
...
export interface UpgradeSmartComponentProps {
  orgId: string;
  promptId: PromptId;
...
const formatAttachmentSizeLimit = forTemplate('attachment_size_limit');

export const UpgradeSmartComponentNoBoundary: React.FC<UpgradeSmartComponentProps> = ({
  orgId,
  promptId,
  additionalProps,
  allowUpsell,
  ...
  const configs: Record<PromptId, React.ReactElement | null> = {
    attachmentSizeLimitUpgradePromptFull: (
      <UpgradePromptFullBase
        ...
        content={formatAttachmentSizeLimit(
          'that-file-size-exceeds-the-10mb-limit',
        )}
        cta={
          <button
            data-test-id={UpgradePromptTestIds.AttachmentSizeLimitUpgradePrompt}
            ...
            onClick={() => {
              Analytics.sendUIEvent({
                ...
                attributes: {
                  ...
                  promptId: 'attachmentSizeLimitUpgradePromptFull',
                },
              });
              openPlanSelection();
            }}
          >
            {isEligible
              ? formatAttachmentSizeLimit('start-free-trial')
              : formatAttachmentSizeLimit('upgrade-now')}
          </button>
        }
    ...
    approachingBoardLimitTemplateUpgradePromptPill: (
      <UpgradePromptButtonBase
        cta={
          isEligible
            ? formatCreateBoard('start free trial')
            : formatCreateBoard('upgrade')
        }
        // eslint-disable-next-line react/jsx-no-bind
        ctaOnClick={() => {
          ...
          if (!additionalProps) {
            // retain previous functionality so nothing breaks
            openPlanSelection();
          } else {
            // If a `hideOverlay` function was passed down, call it so it doesn't block the modal that gets opened
            additionalProps.hideOverlay && additionalProps.hideOverlay();
            // open free trial (openPlanSelectionOverlay) or open plan comparison modal (openPlanSelection)
            isEligible && additionalProps.openPlanSelectionOverlay
              ? additionalProps.openPlanSelectionOverlay()
              : openPlanSelection();
          }
        }}
      />
      ...
      <UpgradePromptFullBase
        ...
        cta={
          <button
            ...
            className={classNames({
              [styles.cta]: true,
              [styles.lightModeCta]: true,
              [styles.disabledCta]: isStandard && !isAdmin,
            })}
            disabled={isStandard && !isAdmin}
            // eslint-disable-next-line react/jsx-no-bind
            onClick={() => {
              Analytics.sendUIEvent({
                action: 'clicked',
          ...
          >
            {isStandard && !isAdmin
              ? formatUpgradePrompt('upgrade contact admin')
              : isEligible
              ? formatFreeTrial(['upgrade prompt', 'cta-ft-repackaging-gtm'])
              : formatUpgradePrompt('upgrade now')}
          </button>
        }
      />
    ),
...
export const UpgradeSmartComponent: React.FC<UpgradeSmartComponentProps> = (
  props,
) => (
  <ErrorBoundary
    tags={{
...
export const UpgradeSmartComponentConnected: React.FC<UpgradeSmartComponentProps> = (
  props,
) => (
  <ComponentWrapper>
