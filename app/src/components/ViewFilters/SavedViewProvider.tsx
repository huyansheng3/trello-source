...
export const SavedViewProvider: React.FunctionComponent<SavedViewProviderProps> = ({
  ...
  // This is technically `clearNonBoardFilters`, as it doesn't (yet) implement the `skip` API.
  const clearFilters: ViewFiltersSourceEditable['clearFilters'] = useCallback(() => {
    if (!data?.organizationView || !view || !cardFilterCriteria || loading) {
      return;
    }
  ...
  }, [idWorkspaceView, cardFilterCriteria, setFilter, clearFilters]);
