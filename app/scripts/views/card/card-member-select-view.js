...
class CardMemberSelectView extends View {
  ...
  initialize() {
    ...
    Analytics.sendScreenEvent({
      name: 'cardMemberSelectInlineDialog',
  ...
  renderSuggested() {
    ...
    if (showSuggestions) {
      ...
      Analytics.sendViewedComponentEvent({
        ...
        componentName: 'suggestedMembersSection',
        source: 'cardMemberSelectInlineDialog',
        attributes: {
          totalSuggestedMembers: suggestedMembers.length,
        },
        containers: {
            ...
            if (res) {
              Analytics.sendUpdatedCardFieldEvent({
                ...
                containers: {
                  ...
                  organization: { id: model.getBoard()?.get('idOrganization') },
                },
                attributes: {
                  taskId: traceId,
        ...
        (_err, res) => {
          if (res) {
            Analytics.sendUpdatedCardFieldEvent({
              ...
              attributes: {
                taskId: traceId,
                changeType: Auth.isMe(idMember) ? 'join' : 'add member',
              },
            });
          }
        },
