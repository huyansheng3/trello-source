...
import {
  classForDueDate as getClassForDueDate,
  relativeInfoForDueDate,
  titleForDueDate,
} from '@trello/dates';
...
export const renderDueDate = function () {
  ...
  const relativeDueDateStatus = relativeInfoForDueDate(due, dueComplete);
  const classForDueDate = getClassForDueDate(due, dueComplete);
  const classes = [];
  ...
  const $badge = this.$('.js-card-detail-due-date-badge')
    ...
    .attr('title', titleForDueDate(due, dueComplete));
  ...
  if (openDateRangePickerOnDateBadges) {
    ...
    const headerString = this.model.get('start')
...
export const renderDateRangePickerButton = function () {
  ...
  if (container && !isTemplate) {
    ...
    this.$('.js-date-range-picker').addClass('button-link-container');
