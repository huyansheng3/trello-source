...
const { ExternalLinkIcon } = require('@trello/nachos/icons/external-link');
const { forTemplate } = require('@trello/i18n');

const format = forTemplate('popup_blocked');
  ...
  t.div(
    ...
    function () {
      if (hasLargeAttachments) {
        ...
        t.div('.attachment-size-limit-upgrade-prompt');
      }
    },
  );
...
class AttachmentTypePickerView extends View {
  ...
  render() {
    ...
    const container = this.$('.attachment-size-limit-upgrade-prompt')[0];
    this.unmountAttachmentUpgradePrompt = container
      ? renderComponent(
          <UpgradeSmartComponent
            orgId={org?.id}
            promptId="attachmentSizeLimitUpgradePromptFull"
            allowUpsell={!dontUpsell()}
          />,
          container,
        )
      : undefined;
  ...
  remove() {
    this.unmountAttachmentUpgradePrompt?.();
  }
      ...
      .catch((err) => {
        switch (err.message) {
          ...
          case 'popup_blocked_by_browser':
            // The browser's popup blocker prevented opening the singin window
            // On first click of "Google Drive", Safari and Firefox throw this error. Second click opens popup
            showFlag({
              id: 'googleSignInError',
              title: format('google-sign-in-error'),
              description: (
                <>
                  <div>{format('try-again-or-support')}</div>
                  <div>
                    <a
                      href="https://help.trello.com/article/1331-trouble-attaching-from-google-drive"
                      target="_blank"
                    >
                      {format('visit-support')} <ExternalLinkIcon />
                    </a>
                  </div>
                </>
              ),
          ...
          case 'idpiframe_initialization_failed':
            // When not currently signed in to a Google account, Chrome throws this error everytime and will not open the sign in window
            showFlag({
              id: 'googleSignInError',
              title: format('google-sign-in-error'),
              description: (
