...
const { LazyBoardProfileCard } = require('app/src/components/ProfileCard');
const { featureFlagClient } = require('@trello/feature-flag-client');
const { Analytics, tracingCallback } = require('@trello/atlassian-analytics');

const removeMemberFromBoardFunction = (board, member) => {
  const source = 'boardMemberInlineDialog';
  const traceId = Analytics.startTask({
    taskName: 'edit-board/members/remove',
    source,
  });

  board.removeMemberWithTracing(
    member,
    traceId,
    tracingCallback(
      {
        ...
        ? {
            reactElement: (
              <LazyBoardProfileCard
                ...
                workspaceId={this.board.getOrganization()?.id}
                // eslint-disable-next-line react/jsx-no-bind
                removeMemberFromBoardOverride={() =>
                  removeMemberFromBoardFunction(this.board, this.model)
                }
                // eslint-disable-next-line react/jsx-no-bind
                onBack={
                  this.options.pushView ? () => PopOver.popView() : undefined
                }
              />
            ),
            hideHeader: true,
