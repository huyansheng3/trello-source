  ...
  loadOrg(orgId) {
    return BluebirdPromise.try(() => {
      if (orgId) {
        ...
        // see if the org exists in the cache and contains all the data that we need
        // otherwise, go ahead and get the info from the api.
        if (org?.get('memberships') && org?.get('prefs')) {
          return org;
        } else {
