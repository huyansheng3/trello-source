...
const {
  DefaultBoardClosedView,
} = require('app/src/components/BoardClosed/DefaultBoardClosedView');
const { BoardClosed } = require('app/src/components/BoardClosed/BoardClosed');
...
class BoardView extends View {
  static initClass() {
    ...
    this.prototype.renderBritaBoardFilterButton = renderBritaBoardFilterButton;
  ...
  onShortcut = (event) => {
    ...
    switch (key) {
      case Key.f:
        if (!isBoardHeaderFilterEnabled && !isProjectBritaToBoardEnabled) {
          this.toggleFilter();
        }
        return;
      case Key.q:
  ...
  renderClosed() {
    const isWorskspaceDeletionBoardReopenEnabled = featureFlagClient.get(
      'mocha.ws-deletion-board-reopen-web',
      false,
    );
    ...
    if (
      (props.isAtOrOverLimit && props.canReopen) ||
      !isWorskspaceDeletionBoardReopenEnabled
    ) {
      this.unmountBoardClosed = renderComponent(
        <DefaultBoardClosedView {...props} />,
        this.$el[0],
      );
      this.defaultBoardBackground();
    } else {
      this.unmountBoardClosed = renderComponent(
        <BoardClosed boardId={this.model.get('id')} />,
        this.$el[0],
      );
