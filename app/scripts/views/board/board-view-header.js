...
} = require('app/src/components/ViewsGenerics/canViewTeamTablePage');
...
const { FilterPopoverButton } = require('app/src/components/FilterPopover');
const {
  SingleBoardViewProvider,
} = require('app/src/components/BoardViewContext/SingleBoardViewProvider');
const {
  SingleBoardViewFiltersProvider,
} = require('app/src/components/ViewFilters/SingleBoardViewFiltersProvider');
...
module.exports.renderBoardHeader = function () {
  ...
  const britaReactRoot = this.el.querySelector(
    '.js-brita-board-filter-btn-container',
  );
  ...
  const newBritaReactRoot = this.el.querySelector(
    '.js-brita-board-filter-btn-container',
  );
  ...
  if (britaReactRoot) {
    newBritaReactRoot.parentNode.replaceChild(
      britaReactRoot,
      newBritaReactRoot,
    );
  }
  ...
  const isProjectBritaToBoardEnabled = featureFlagClient.get(
    'panorama.project-brita-on-boards',
    false,
  );

  if (isProjectBritaToBoardEnabled) {
    this.renderBritaBoardFilterButton();
  }
...
module.exports.renderBritaBoardFilterButton = function () {
  ...
  renderComponent(
    <SingleBoardViewFiltersProvider idBoard={this.model.id}>
      <SingleBoardViewProvider idBoard={this.model.id}>
        <FilterPopoverButton
          appearance={
            this.model.getPref('backgroundBrightness') === 'dark'
              ? 'transparent'
              : 'transparent-dark'
          }
          className={'board-header-filter-btn'}
        />
      </SingleBoardViewProvider>
    </SingleBoardViewFiltersProvider>,
    targetNodeForButton,
