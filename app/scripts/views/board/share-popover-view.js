...
const { makeSlug } = require('@trello/urls');
...
} = require('app/src/components/UpgradePrompts');
...
class SharePopoverView extends View {
  ...
  render() {
    ...
    const isStandard = org && org.isStandard();
    const billingUrl = this.getBillingUrl(org);
    const orgId = org != null ? org.id : undefined;

    const shouldShowUpgradePrompt =
      !data.isAdDismissed &&
      // make sure prompt shows for free and standard
      (isFreeTeam || isStandard) &&
      !data.hasCsvExport &&
      data.upsellEnabled;
  ...
  async tracedExportCSV(e) {
    ...
    try {
      ...
      const slug = makeSlug(this.model.get('name'));
