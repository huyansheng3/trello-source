...
const { Analytics, formatContainers } = require('@trello/atlassian-analytics');
...
const { DueFilter } = require('app/src/components/ViewFilters/filters');

class CardFilterView extends View {
  static initClass() {
    this.prototype.viewTitleKey = 'search cards';
  ...
  toggleLabelFilter(e) {
    ...
    Analytics.sendUIEvent({
      ...
      containers: formatContainers({
        idBoard: this.model.board.id,
        idOrganization: this.model.board.getOrganization()?.id,
        idEnterprise: this.model.board.getEnterprise()?.id,
      }),
      attributes: {
        type: 'label',
        id: idLabel,
  ...
  toggleDateFilter(e) {
    ...
    switch (newDateFilter) {
      case 'complete':
        return DueFilter.setBoardDueFilter('dueComplete', true, this.model);
      case 'incomplete':
        return DueFilter.setBoardDueFilter('dueComplete', false, this.model);
      case 'overdue':
        return DueFilter.setBoardDueFilter('overdue', true, this.model);
      default: {
        return DueFilter.setBoardDueFilter('due', newDateFilter, this.model);
      }
    }
  }
