...
const {
  ...
  fetchUsersFromAllSources,
  getSlackConnection,
  InviteFromSlackCaptionLink,
  InviteFromSlack,
  HideSlackUsersPopOver,
...
class BoardMemberAddMultipleView extends View {
  ...
  initialize() {
    ...
    this.onSlackConnectionLinkClick = this.onSlackConnectionLinkClick.bind(
      this,
    );
      ...
      if (trelloInviteFromSlackFeatureFlag === 'not-enrolled') {
        ...
        if (trelloInviteFromSlackFeatureFlag === 'experiment') {
          getSlackConnection(this.model.get('shortLink'), document.cookie).then(
            (res) => {
              this.hasSlackConnection = res.associations;
              this.stateSlackOauth = res.state;
    ...
    _.forEach(this.selectedMemberOptions, (option) => {
      ...
      if (member) {
        ...
        if (this.isInviteFromSlackExperiment) {
          ...
          return users.push({ value, type, source, traceId });
        }
  ...
  onAutocompleteSearch(query) {
    ...
    if (query.length > 2) {
      if (isEligibleForExperiment && this.hasSlackConnection) {
        return fetchUsersFromAllSources(
          query,
          org != null ? org.id : undefined,
          this.model.id,
          document.cookie,
  ...
  onSlackConnectionLinkClick() {
    Analytics.sendClickedButtonEvent({
      buttonName: 'connectedToSlackLink',
      source: 'inviteToBoardInlineDialog',
      attributes: {
    ...
    PopOver.pushView({
      reactElement: (
        <HideSlackUsersPopOver
          key="trelloInviteFromSlackHideSlackUser"
          idBoard={this.model.id}
        />
      ),
      getViewTitle: () => {
  ...
  renderAutocomplete() {
    ...
    const autocompleteProps = {
      ...
      idBoard: this.model.id,
      isInviteFromSlackExperiment: this.isInviteFromSlackExperiment,
      displayDetails: true,
    ...
    if (this.isInviteFromSlackExperiment) {
      children.push(
        <InviteFromSlackCaptionLink
          ...
          onSlackConnectionLinkClick={this.onSlackConnectionLinkClick}
          key="trelloInviteFromSlackCaptionLink"
        />,
      );
    }

    if (this.model?.isFeatureEnabled('observers')) {
      children.push(
      ...
      <MemberMultiSelectAutocompleteComponent {...autocompleteProps}>
        ...
        {this.isInviteFromSlackExperiment ? (
          <InviteFromSlack
            idBoard={this.model.id}
            hasSlackConnection={this.hasSlackConnection}
            key="trelloInviteFromSlackButton"
            stateSlackOauth={this.stateSlackOauth}
            renderAutocomplete={this.render}
          />
        ) : null}
