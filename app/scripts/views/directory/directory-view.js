...
const {
  mapPowerUpId: MAP_POWER_UP_ID,
  e2bId,
  customFieldsId: CUSTOM_FIELDS_ID,
...
const getPageName = function (section, category, idPlugin, search) {
  ...
  if (search) {
    ...
    page = 'list page';
  } else if (['enabled', 'custom', 'made-by-trello'].includes(section)) {
    page = section + ' page';
...
const CreateUnlimitedPupDirectoryAnnouncement = () => {
  const UnlimitedPupDirectoryAnnouncement = useLazyComponent(
    () =>
      import(
        /* webpackChunkName: "unlimited-pup-directory-announcement" */ 'app/src/components/PowerUpDirectory/UnlimitedPupDirectoryAnnouncement'
      ),
    {
      namedImport: 'UnlimitedPupDirectoryAnnouncement',
      preload: false,
    },
  );
  ...
  return (
    <React.Suspense fallback={null}>
      <UnlimitedPupDirectoryAnnouncement />
    </React.Suspense>
  );
};
...
class DirectoryView extends View {
  ...
  getEnabledPlugins() {
    ...
    return _.chain(combinedPlugins)
      .filter((idOrName) => {
        const repackagingPupsFlag = featureFlagClient.get(
        ...
        return (
          ...
          (!repackagingPupsFlag || idOrName !== CUSTOM_FIELDS_ID)
        );
      })
      .map((idOrName) => {
  ...
  renderOnce() {
    ...
    if (
      featureFlagClient.get('ecosystem.repackaging-pups', true) &&
      !Auth.me().isDismissed('unlimited-pup-directory-announcement') &&
      Auth?.me()?.getMaxPaidStatus() === 'free'
    ) {
      this.$directoryBody.append('<div class="directory-announcement"></div>');
      renderComponent(
        <CreateUnlimitedPupDirectoryAnnouncement />,
        this.$('.directory-announcement')[0],
      );
    }
    ...
    this.$directoryBody.append(
      '<div class="directory-flex-body-no-wrap"></div>',
    );
    this.$directoryBodyNoWrap = this.$('.directory-flex-body-no-wrap');

    this.$directoryBody.append(
      '<div class="directory-responsive-overlay js-directory-responsive-overlay"></div>',
    );

    this.appendSubview(this.sidebarView, this.$directoryBodyNoWrap);

    this.$directoryBodyNoWrap.append('<div id="directory-content"></div>');
